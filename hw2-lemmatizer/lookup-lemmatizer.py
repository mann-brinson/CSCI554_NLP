...
for key in lemma_count.keys():
    ...
    if len(value) > 1: 
        ...
        if key in value: training_counts['Identity tokens'] += value[key]
    else: 
        training_counts['Unambiguous tokens'] += sum(value.values())
        training_counts['Unambiguous types'] += 1
        
        #check if its an identity token
        if key in value: training_counts['Identity tokens'] += sum(value.values())
...
for line in test_data:
    ...
    if re.search ('\t' , line):
        ...
        if form in lemma_max: 
            ...
            if lemma_predict == lemma_true: test_counts['Lookup match'] += 1
            else: test_counts['Lookup mismatch'] += 1
        else: 
            lemma_predict = form
            test_counts['Not found in lookup table'] += 1
            if lemma_predict == lemma_true: test_counts['Identity match'] += 1
            else: test_counts['Identity mismatch'] += 1
