        ...
        while True:
            ...
            if phrase not in self.emis_inv: #word completely unseen
                for tag1 in self.ow_tags:
                    a = log10(self.trans_emis["$START"]['next'][tag1]) - log10(self.trans_emis["$START"]['count'])
                    prob_mat[t][tag1] = a
                    back_mat[t][tag1] = '$START'
            else: #word seen 
                for tag1 in self.tags:
                    if tag1 not in self.emis_inv[phrase]['tag_0']: #word-tag pair unseen
                        continue
                    else: #word-tag pair seen
                        a = log10(self.trans_emis["$START"]['next'][tag1]) - log10(self.trans_emis["$START"]['count'])
                        b = log10(self.emis_inv[phrase]['tag_0'][tag1]) - log10(self.trans_emis[tag1]['count'])
                        prob_mat[t][tag1] = a + b
                        back_mat[t][tag1] = '$START'         
            #end :: first phrase
                ...
                if phrase not in self.emis_inv: #word completely unseen
                    for tag1 in self.ow_tags:
                        ...
                        for tag0 in prob_mat[t]: #look at previous tags
                            ...
                            a = log10(self.trans_emis[tag0]['next'][tag1]) - log10(self.trans_emis[tag0]['count'])
                            prob_score = prior + a
            ...
            for t in reversed(range(2, len(line))):
                ...
                mlt = mlt_next
            line.pop()
            res = ' '.join([f'{phrase}/{tag}' for phrase, tag in zip(line, state_seq)])
            result.append(res)
        return result
